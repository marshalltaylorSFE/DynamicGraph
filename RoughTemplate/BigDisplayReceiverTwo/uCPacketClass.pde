	...
	int parseSingle()  //parse and remove all valid packets, keep newest
	{
		...
		else
		{
			...
			if(( index != -1 )&&(sBuffer.length() > 25))
			{
				...
				sBuffer.delete(0, index);
				//println(pBuffer);
				knob1 = unhex(pBuffer.substring(3,5) + pBuffer.substring(1,3));
				knob2 = unhex(pBuffer.substring(7,9) + pBuffer.substring(5,7));
				knob3 = unhex(pBuffer.substring(11,13) + pBuffer.substring(9,11));
				knob4 = unhex(pBuffer.substring(15,17) + pBuffer.substring(13,15));
				knob5 = unhex(pBuffer.substring(19,21) + pBuffer.substring(17,19));
