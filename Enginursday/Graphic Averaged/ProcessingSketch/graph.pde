...
class pGraph
{
	pGraph()
	{
		...
		randomKnob.traceColor[0] = 27;
		randomKnob.traceColor[1] = 106;
		randomKnob.traceColor[2] = 209;
	...
	void postDraw()
	{
		stroke(102, 191, 1);
		strokeWeight(2);
		int y1 = y_origin + y_size - (int)((var1 - y_offset)* y_scale);
		if( y1 < y_origin ) y1 = y_origin;
		if( y1 > y_origin + y_size ) y1 = y_origin + y_size;
		line( x_origin, y1, x_origin + x_size, y1 );
		y1 = y_origin + y_size - (int)((var2 - y_offset)* y_scale);
		if( y1 < y_origin ) y1 = y_origin;
		if( y1 > y_origin + y_size ) y1 = y_origin + y_size;
		line( x_origin, y1, x_origin + x_size, y1 );
		rawInput.postDraw();
